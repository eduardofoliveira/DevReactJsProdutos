...
class Produtos extends Component {
  ...
  renderCategoria(cat){
    return(
      <li key={cat.id} className="list-group-item">
        ...
        {this.state.editingCategoria !== cat.id &&
        <div>
          <button className="btn btn-danger btn-sm" onClick={() => this.props.removeCategoria(cat)}>
            <span className="glyphicon glyphicon-remove"></span>
          </button>
          <button className="btn btn-danger btn-sm btn-margin" onClick={() => this.editCategoria(cat)}>
  ...
  render() {
    let { match, categorias } = this.props
    return(
      <div className="row">
        <div className="col-md-3">
          ...
          <Link to={'/produtos/novo'} className="btn btn-success"><span className="glyphicon glyphicon-plus"></span> Novo produto</Link>
        </div>
        <div className="col-md-9">
          ...
          />
          <Route path={match.url+'/categorias/:catId'}
            render={(props) => {
